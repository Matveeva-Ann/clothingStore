...
import { AccentText, AddBasketBtn, GoodCategory, GoodDescriptionItem, GoodImg, GoodInformation, GoodsColorsItem, GoodsColorsList, GoodsDescriptionList, GoodWrapper, InputRadio, PropertyName } from "./GoodInfo.styled";
...
export default function GoodInfo({ setFavoriteCount, setBasketCount }) {
  ...
  function checkFavorite(cardData) {
    const lSFavorite = JSON.parse(window.localStorage.getItem('favorite')) || [];
  ...
  function addToBasket() {
    ...
    let newBasketArr = isInBasket
       ? lSBasket.map (elem => (elem.sku === good.sku ? {...elem, amount: elem.amount+1, color: color} : elem))
        ...
        <GoodInformation>
          ...
          <GoodsDescriptionList>
            {good.description.map((elem, index) => {
              return <GoodDescriptionItem key={index}>{elem}</GoodDescriptionItem>
            })
            }
          </GoodsDescriptionList>
          <PropertyName><AccentText>Колір:</AccentText> {color}</PropertyName>
          <div> Варіанти кольорів:
            <GoodsColorsList>
              {good.colors.map((elem, index) => (
                <li key={index} title={elem.name} onClick={() => clickOnCalor(elem)} style={{ listStyle: 'none' }} >
                  <InputRadio type="radio" name="color" className="inputRadio" id={index} defaultChecked={elem.isChecked} />
                  <GoodsColorsItem colorObj={elem} className='goodsColorsItem' htmlFor={index}></GoodsColorsItem>
                </li>
              ))}
            </GoodsColorsList>
          </div>
          <PropertyName><AccentText>Ціна:</AccentText> ${good.price}</PropertyName>

          <IconsWrapper>
            <IconButton onClick={() => toggleFavorite(good.sku)} background='white' ariaLabel={'add to favorite'}>
              {isFavorite
            ...
            <AddBasketBtn onClick={() => setIsOpenModal(!isOpenModal)}>
              <BsFillBasket3Fill style={{ color: '#FFFFFF', marginRight: '10px' }} size='16px'></BsFillBasket3Fill> Додати в кошик
            </AddBasketBtn>

          </IconsWrapper>
        </GoodInformation>
        {isOpenModal &&
          <ModalAddToBasket onClickBasket={addToBasket} onClick={() => setIsOpenModal(!isOpenModal)} good={good}></ModalAddToBasket>
        }
      </GoodWrapper>
    )
  );
